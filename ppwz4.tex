\chapter{Zestaw 4, Jêzyk C\# 3.0}

\begin{flushright}
Liczba punktów do zdobycia: {\bf 10/36}
\end{flushright}

{\em Uwaga! W zadaniach w których polecenie brzmi "dany jest plik tekstowy ..." nale¿y sobie we w³asnym zakresie
przygotowaæ taki przyk³adowy plik tekstowy.}

\begin{enumerate}
  
\item ({\bf 1p}) Zaimplementowaæ metodê {\tt bool IsPalindrome()} rozszerzaj¹c¹ klasê {\tt string}. Implementacja powinna byæ niewra¿liwa na bia³e znaki i znaki
...
\item ({\bf 1p}) Dany jest plik tekstowy zawieraj¹cy zbiór liczb naturalnych w kolejnych liniach.
...
\item ({\bf 1p}) Dany jest plik tekstowy zawieraj¹cy zbiór nazwisk w kolejnych liniach.
...
\label{linq_grupowanie}  
  ...
  {\em Wskazówka: zgodnie z tytu³em zadania u¿yæ operatora GroupBy }

\item ({\bf 1p}) Napisaæ wyra¿enie LINQ, które dla zadanego foldera wyznaczy sumê
...
\item ({\bf 1p}) Dane s¹ dwa pliki tekstowe, pierwszy zawieraj¹cy zbiór danych osobowych postaci
...
\item ({\bf 2p}) Rejestr zdarzeñ serwera IIS ma postaæ pliku tekstowego, w którym ka¿da
\label{linq_logi}  
  linia ma postaæ:
...
\item ({\bf 1p}) Listy generyczne ukonkretnieniamy typem elementów:
...
\item ({\bf 2p}) Cech¹ charakterystyczn¹ anonimowych delegacji, bez wzglêdu na to czy zdefiniowano je przy u¿yciu s³owa
